...
import detImage from '../assets/images/det.jpg';
import { detectiveAPI } from '../api/auth';
import { gameAPI } from '../api/game';

const DetectiveLogin: React.FC = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (token && role === 'detective') {
        try {
          // Verify token is valid
          const response = await fetch(`${import.meta.env.VITE_API_URL}/api/detective/profile`, {
            headers: {
          ...
          if (response.ok) {
            // Token is valid, check contest status
            const gameState = await gameAPI.getState();
            if (gameState.success && gameState.gameState.contestStarted) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Login via API
      const response = await detectiveAPI.login(form.username, form.password);

      if (response.success) {
        // Store token and role in localStorage
        localStorage.setItem('authToken', response.token);
        localStorage.setItem('userRole', response.role || 'detective');
        
        // Store user data in sessionStorage
        sessionStorage.setItem('detectiveData', JSON.stringify(response.user));
  ...
  return (
    <div className="w-screen h-screen relative overflow-hidden">
      ...
      {/* Main form container - styled as case file on desk */}
      <div className="relative w-screen h-screen flex items-center justify-center p-8">
        <div
          className={`relative bg-gradient-to-b from-amber-50 via-amber-100 to-amber-50 border-4 border-amber-800 p-10 shadow-2xl max-w-lg w-full transition-all duration-700 ${
            mounted ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-90 rotate-1'
          }`}
          style={{
            boxShadow: '0 20px 60px rgba(0, 0, 0, 0.5), 0 0 0 1px rgba(139, 0, 0, 0.1)',
          }}
        >
          ...
          {/* Red pushpin - like in the case file */}
          <div className="absolute -top-2 left-8 w-4 h-4 bg-noir-red rounded-full shadow-lg border-2 border-amber-800"></div>
          <div className="absolute top-2 left-8 w-0.5 h-3 bg-amber-800"></div>

          {/* Case file header */}
          <div className="relative mb-6 pb-4 border-b-2 border-amber-700">
            <div className="text-xs font-mono text-amber-900 opacity-60 mb-1 tracking-widest uppercase">
              Case File #001
            </div>
            <h1
            ...
            <div className="text-xs font-mono text-amber-700 mt-2 italic">
              Confidential Investigation Form
            </div>
          </div>
            ...
            <div className={`pt-4 transition-all duration-700 delay-400 ${
              ...
              >
                <span className="relative z-10">
                  {isSubmitting ? 'PROCESSING...' : 'LOGIN & ENTER'}
                </span>
          ...
          <div className="absolute bottom-6 right-6 text-noir-red text-xs font-mono font-bold opacity-50 transform rotate-12 border-2 border-noir-red px-2 py-1 bg-white bg-opacity-80">
            CLASSIFIED
          </div>
          ...
          {/* Typed text lines effect - subtle */}
          <div className="absolute bottom-20 left-10 right-10 h-px bg-amber-300 opacity-20"></div>
          <div className="absolute bottom-16 left-10 right-16 h-px bg-amber-300 opacity-20"></div>
        </div>
      </div>
    </div>
  );
